---
layout: single
title:  "machine learning 기본 상식"
---
# machine learning 기본 상식
...
### 행렬의 덧셈은 같은 위치의 요소끼리 더해주면 된다
그러므로 더할 수 있으려면 같은 차원이어야 한다

### 행렬과 스칼라의 곱셈은 각 원소에 곱해주면 된다
...
### 행렬 간의 곱셈은 내적곱과 외적곱으로 나뉜다
행렬 곱셈은 순서에 따라 결과가 다르다  
차원에 따라 곱셈 가능여부도 다르다
...
### 내적곱, 외적곱 외에도 요소별 곱하기가 있다

#### 요소별 곱하기 element-wise multiplication A∘B 아다마르 곱(Hadamard product)
...
- A(i,j)의 전치행렬 transposal matrix는 At(j,i)이다. 각 행을 열로 바꾼 행렬이다.
- A(i,j)와 그 전치행렬 At(j,i)가 동일하다면 A는 **대칭행렬**이라 부른다
...
- 기울, 평균변화율, 순간변화율, 미분 차례로 설명하니 이해가 쉽다

[f(b) - f(a)] / (b - a) x 증가량 분의 y 증가량을 표현하면 이렇게 된다 (y는 x에 대한 함수라고 가정)  
if b = a + h 식을 간단하게 표시하기 위해 분모를 정리해준다  
[f(a+h) - f(a)] / h  
for a general case, replace a with x 상수 a를 미지수 x로 치환해서 일반화 한다  
[f(x+h) - f(x)] / h  
limit h -> 0, 위 식에서 h를 극한으로 0값에 가깝게 하면 접선의 기울기를 알 수 있다  
이 때 f(x)를 풀어서 정리하고 h를 0으로 보내야 한다  
그렇게 정리하면 식이 나오는데 이를 f'(x)라 쓰고 f prime이라 읽으며 이를 f(x)의 미분이라 한다 이 식에 x에 값을 넣으면 나오는 결과는 x일 때의 기울기이다. 즉 f'(x) 미분은 함수의 순간 변화율을 구하는 함수다
...
- 어떤 방향으로 가야 가장 가파르게 "올라갈 수 있는가"
  - 음수면 숫자가 작아지는 방향
  - 양수면 숫자가 커지는 방향
  - 즉, 절대값이 커지는 방향으로 가야 가파르게 "올라갈 수 있다"
...
## 편미분
변수가 2개 이상이 될 때, 즉 3차원 이상일 때에 대한 미분
