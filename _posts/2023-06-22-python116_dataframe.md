...
행렬 (행, 렬) 표로 나타냈을 때 xy로 표현하면 (y, x)가 된다. 가로 세로로 보면 세로인 행이 높이니까. 천재
...
칼럼 여러개 가져오려면 df.loc[:, ['a', 'b']] 이렇게 콜론까지 찍어줘야 한다
...
### 인덱스를 지정하면 기존 인덱스가 날아가니 살리려면 작업해야 한다
