---
layout: single
title:  "Objective Oriented Programming 객체 지향 프로그래밍"
---
...
위 예제에서 볼 수 있듯이 클래스 매소드는 첫번째 인자로 cls를 받는다. cls는 클래스 자체를 의미한다.  
...
스태틱 매소드는 클래스와 인스턴스 그 어떤 객체도 사용하지 않는 매소드이다. 사실상 외부 함수랑 동일한데 클래스 내에 정의되어 있을 뿐이다.  
클래스나 인스턴스의 정보를 사용하진 않지만, 클래스가 수행하는 기능에 속한다고 판단 될 때, 즉 기능을 일관되게 정리하고자 할 때 사용한다.  
데코레이터는 @staticmethod이며 위의 예제에서 같이 볼 수 있다

---

## first class function 일급함수 tbd
